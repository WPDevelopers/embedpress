...
class OpenSea extends ProviderAdapter implements ProviderInterface {
    ...
    public function normalizeJSONData($asset){
        ...
        $nftItem['last_sale'] = isset($last_sale)?(float)($last_sale[0]->current_price / 1000000000000000000) : 0;
        $nftItem['creator_url'] = 'https://opensea.io/'.$nftItem['created_by'];

        //single asstet data
        $nftItem['collection_name'] = isset($asset->collection->name)?$asset->collection->name:'';
     ...
     */
     public function nftSingleItemTemplate($item){
        ...
        $rank = '';
        if(!empty($item['rank'])){
            $rank = '<a target="_blank" href="'.esc_url($item['permalink']).'" '.$this->createStye('buttonTextColor', 'buttonFontSize', 'buttonBackgroundColor').'>Rank #'.esc_html($item['rank']).'</a>';
        }

        $collection_name = '<a class="CollectionLink--link" href="'.esc_url('/collection/'.$item['collection_slug']).'"><span
        class="CollectionLink--name">'.esc_html($item['collection_name']).$is_verified.'</span></a>';
        ...
        if(!empty($current_price) &&  (($current_price > 0) && (($params['nftprice'] == 'yes') || ($params['nftprice'] == 'true')))){
            ...
            <div class="ep_nft_price ep_current_price" '.$this->createStye('priceColor', 'priceFontsize', '').'>
                <span class="eb_nft_label">'.esc_html($prefix_current_price).'</span>
                <span class="eb_nft_price">'. esc_html(round($current_price, 4)).' ETH <sub class="ep-usd-price">$'.$item['usd_price'].'</sub></span>
            </div>
            ';
        }
        ...
        if(!empty($last_sale) && (($last_sale > 0) && (($params['nftlastsale'] == 'yes') || ($params['nftlastsale'] == 'true')))){
            ...
            <div class="ep_nft_price ep_nft_last_sale" '.$this->createStye('lastSaleColor', 'lastSaleFontsize', '').'>
                <span class="eb_nft_label">'.esc_html($prefix_last_sale).'</span>
                <span class="eb_nft_price">'. esc_html(round($last_sale, 4)).' ETH <sub class="ep-usd-price">$'.$item['usd_price'].'</sub></span>
            </div>
            ';
        }
