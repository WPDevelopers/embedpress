    ...
    public function get_milestone_message($type, $milestone_value, $achieved_value)
    {
        $messages = [
            'total_views' => sprintf(
                ...
                esc_html__('🎉 Congratulations! Your embedded content has reached %s total views! Consider upgrading to EmbedPress Pro for advanced analytics and features.', 'embedpress'),
                number_format($milestone_value)
            ),
            'total_embeds' => sprintf(
                /* translators: %s: milestone count. */
                esc_html__('🚀 Amazing! You\'ve created %s embedded content pieces! Unlock more embedding options with EmbedPress Pro.', 'embedpress'),
                number_format($milestone_value)
            ),
            'daily_views' => sprintf(
                /* translators: %s: milestone count. */
                esc_html__('📈 Fantastic! You\'ve reached %s views today! Upgrade to Pro for detailed daily analytics and insights.', 'embedpress'),
                number_format($milestone_value)
            ),
            'monthly_views' => sprintf(
                /* translators: %s: milestone count. */
                esc_html__('🏆 Incredible! %s views this month! Get advanced monthly reports with EmbedPress Pro.', 'embedpress'),
                number_format($milestone_value)
            )
        ];

        return isset($messages[$type]) ? $messages[$type] : esc_html__('Milestone achieved!', 'embedpress');
    }
}
