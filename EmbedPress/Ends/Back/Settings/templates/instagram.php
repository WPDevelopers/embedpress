...
$feed_data = array_filter($feed_data, function ($key) {
    return !empty($key);
        ...
        <div class="account-section">
            <div class="account-wrap full-width-layout">
                <p>
                    ...
                    <?php if (is_array($feed_data) && count($feed_data) > 0) : ?>
                        <button id="open-modal-btn" class="account-button personal-account" target="_self" title="Add Personal Account">
                    ...
                    <tbody>
                        ...
                        <?php if (is_array($feed_data) && count($feed_data) > 0) : ?>
                            <?php foreach ($feed_data as $data) : 
                                ...
                                $data = $data['feed_userinfo'];

                                $avater_url = 'http://2.gravatar.com/avatar/b642b4217b34b1e8d3bd915fc65c4452?s=150&d=mm&r=g';

                                if (!empty($data['connected_account_type']) && (strtolower($data['connected_account_type'])  === 'business')) {
                                    $avater_url = '';
                                }
                                ...
                                <?php if (!empty($data['profile_picture_url'])) :
                                            $avater_url = $data['profile_picture_url'];
                                        else :
                                            $avater_url = 'http://2.gravatar.com/avatar/b642b4217b34b1e8d3bd915fc65c4452?s=150&d=mm&r=g';
                                        endif; ?>

                                <tr data-userid="<?php echo esc_attr($data['id']); ?>" data-accounttype="<?php echo esc_attr($data['connected_account_type']); ?>">
                                    <td class="instagram-user-account">
                                        ...
                                        <div>
                                            ...
                                            <div class="userid"><?php echo esc_html($data['id']) ?></div>
                                        </div>
                                    </td>
                                    <?php
                                            if (!empty($data['id'])) {
                                    ...
                                    <!-- <td>1312</td> -->
                                    <td style="text-transform: uppercase;"><?php echo esc_attr($data['connected_account_type']); ?></td>

                                    <td><button class="user-profile-link" title="<?php echo esc_attr('https://instagram.com/' . $data['username']) ?>">Copy</button></td>

                                    <td class="instagram-sync-data" data-userid="<?php echo esc_attr($data['id'])?>" data-acceess-token="<?php echo esc_attr($data['access_token'])?>" data-account-type="<?php echo esc_attr($data['connected_account_type'])?>">
