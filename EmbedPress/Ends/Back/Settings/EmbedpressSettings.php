...
class EmbedpressSettings
{
	...
	{
		add_menu_page(
			esc_html__('EmbedPress Settings', 'embedpress'),
			'EmbedPress',
			'manage_options',
			$this->page_slug,
			[$this, 'render_settings_page'],
			EMBEDPRESS_URL_ASSETS . 'images/menu-icon.svg',
			64
		);
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Dashboard', 'embedpress'),
			esc_html__('Dashboard', 'embedpress'),
			'manage_options',
			$this->page_slug,
			[$this, 'render_settings_page']
		);
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Branding', 'embedpress'),
			esc_html__('Branding', 'embedpress'),
			'manage_options',
			$this->page_slug . '&page_type=custom-logo',
			[$this, 'render_settings_page']
		);
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Custom Ads', 'embedpress'),
			esc_html__('Custom Ads', 'embedpress'),
			'manage_options',
			$this->page_slug . '&page_type=ads',
			[$this, 'render_settings_page']
		);
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Shortcode', 'embedpress'),
			esc_html__('Shortcode', 'embedpress'),
			'manage_options',
			$this->page_slug . '&page_type=shortcode',
			[$this, 'render_settings_page']
		);
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Settings', 'embedpress'),
			esc_html__('Settings', 'embedpress'),
			'manage_options',
			$this->page_slug . '&page_type=settings',
			[$this, 'render_settings_page']
		);
		...
		if (apply_filters('embedpress/is_allow_rander', false)) {
			add_submenu_page(
				$this->page_slug,
				esc_html__('EmbedPress License', 'embedpress'),
				esc_html__('License', 'embedpress'),
				'manage_options',
				$this->page_slug . '&page_type=license',
				[$this, 'render_settings_page']
			);
		}
		...
		add_submenu_page(
			$this->page_slug,
			esc_html__('EmbedPress Setup Wizard', 'embedpress'),
			esc_html__('Setup Wizard', 'embedpress'),
			'manage_options',
			'embedpress-onboarding',
...
<?php
	...
	public function reorder_submenu_items()
	{
		...
		foreach ($submenu[$this->page_slug] as $item) {
			if (isset($item[0])) {
				if ($item[0] === esc_html__('License', 'embedpress')) {
					$license_item = $item;
					continue; // Skip adding to reordered menu
				} elseif ($item[0] === esc_html__('Analytics', 'embedpress')) {
					$analytics_item = $item;
					continue; // Skip adding to reordered menu
				}
			}
