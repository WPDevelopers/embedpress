...
embedpressDocViewer.epDocumentsViewerController = () => {
  ...
  function handleClick(event) {
    ...
    let fileUrl = match && match[2] ? match[2] : iframeSrc;

    // Detect Clicks on All Icons (including SVG & child elements)
    const popupIcon = event.target.closest('.ep-doc-popup-icon, .ep-doc-popup-icon svg, .ep-doc-popup-icon path, .ep-doc-popup-icon use');
    const printIcon = event.target.closest('.ep-doc-print-icon, .ep-doc-print-icon svg, .ep-doc-print-icon path, .ep-doc-print-icon use');
    const downloadIcon = event.target.closest('.ep-doc-download-icon, .ep-doc-download-icon svg, .ep-doc-download-icon path, .ep-doc-download-icon use');
    ...
    if (popupIcon) {
      ...
      window.open(`https://view.officeapps.live.com/op/view.aspx?src=${fileUrl}&wdOrigin=BROWSELINK`, '_blank');
      ...
      // FIX: Check if the document is actually in fullscreen before exiting
      if (document.fullscreenElement) {
      ...
      if (viwerParentEl.requestFullscreen) {
        viwerParentEl.requestFullscreen().catch(err => console.warn("Fullscreen request failed:", err));
  ...
  document.addEventListener("touchstart", handleFullscreenChange);
