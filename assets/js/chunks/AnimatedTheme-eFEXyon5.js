...
class Root {
  ...
  _updateComputedStyles() {
    ...
    const newFonts = keys(styles).flatMap((k) => {
      var _a;
      return typeof k === "string" && k.startsWith("font") ? [(_a = styles[k]) === null || _a === void 0 ? void 0 : _a.toString()] : [];
    });
...
PercentChart.className = "PercentChart";
PercentChart.classNames = SerialChart.classNames.concat([PercentChart.className]);
class PercentSeries extends Series {
  constructor() {
    super(...arguments);
    this.slicesContainer = this.children.push(Container.new(this._root, { position: "absolute", isMeasured: false }));
    this.labelsContainer = this.children.push(Container.new(this._root, { position: "absolute", isMeasured: false }));
    this.ticksContainer = this.children.push(Container.new(this._root, { position: "absolute", isMeasured: false }));
    this._lLabels = [];
    this._rLabels = [];
    this._hLabels = [];
    this.slices = this.addDisposer(this._makeSlices());
    this.labels = this.addDisposer(this._makeLabels());
    this.ticks = this.addDisposer(this._makeTicks());
  }
  _applyThemes(force = false) {
    const colors = this.get("colors");
    if (colors) {
...
PercentSeries.className = "PercentSeries";
PercentSeries.classNames = Series.classNames.concat([PercentSeries.className]);
...
PieChart.classNames = PercentChart.classNames.concat([PieChart.className]);
...
PieSeries.classNames = PercentSeries.classNames.concat([PieSeries.className]);
...
//# sourceMappingURL=AnimatedTheme-eFEXyon5.js.map
